...
We for both, we should either have no result or the setting is set to 'no', such as 'PasswordAuthentication no". We can
...
If a system is using Nginx for a reverse proxy for example, we can check the permissions on the log files with
...
Or if we wish to reduce the file to be read only by super users
