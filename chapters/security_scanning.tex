...
These configuration files could be as simple as a key-value store, like the SSH daemon uses. It is relatively simple
...
It is clear that this can be done with a Regex expression.

Other software may use a DSL to define configuration. These programs may have behavior too complex to define with a
file as simple as a key-value store and would require extra syntax to concisely define its behavior. The web server
...
Despite this, scanning a configuration file which uses a markup language would not be possible without help from an
...
Software which uses simple configuration files are an easy target for configuration file remediation. After scanning
...
Programs which use a DSL or markup language to define their behavior could also be fixed with a find and replace action,
...
Some isses are too complex or specific to the system to be resolved through automated means. These issues will require
