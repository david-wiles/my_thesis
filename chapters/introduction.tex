...
a project on GitHub which will automatically scan a Docker setup for any issues~\cite{docker_bench_security}.
...
company secrets to an attacker, even if the attacker doesn't have root access on the machine.
...
which allows users full access to a guest VM. The malicious actor could attempt to gain control of a number of VM's on
a single machine and overuse the host's resources to degrade performance of the other guests, especially if another
...
on of the most critical issues for a server machine since it could lead to any number of attacks. A malicous actor could
gain control through a number of ways, but the most well-known is through remote code execution. A bug in a program,
