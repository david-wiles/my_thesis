...
Canary is a generic security scanner and resolver meant to be extendable through creation of independent packages.
...
practices already configured there is not a well known all-purpose program which could be used to securely set up a system.
...
fixed programmatically then the user could run a script to fix the issues. Although this does not guarantee safety
...
--directory to speicify the root directory of the custom package. This command also supports '--auto-fix' to fix
...
Canary is a command-line program written in Scala. It contains a custom argument parser which then intializes specific
...
The check command performs the actual software scanning and issue resolution. Check will use the specified packages and
...
This also makes it more simple for third parties to add security checks for their own programs. An organization which
