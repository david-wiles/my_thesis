...
Kubernetes (K8s) is a very prominent software used to orchestrate hundreds or even thousands of containers across
...
\subsection{Performance and Linux Namespaces}
...
containerization from virtualization. The tools provided by Docker make it very easy to create new containers, share
images, and reproduce the systems used by others. Additionally, container orchestration technologies such as Kubernetes
has made shifting a production system to containers instead of VM's more attractive. These tools often choose sane
defaults for security, but there can still be gaps in this setup. These issues are compounded by the fact that it is
possible to override these defaults for the sake of ease of setup, which can creep into production systems when settings
from development environments are evaluated when copied over.
...
\begin{center}
    \begin{tabular}{||p{0.3\textwidth} p{0.3\textwidth} p{0.3\textwidth}||}
        ...
        \hline
        AppArmor profiles and seccomp configuration & Arbitrary code execution, privilege escalation & Access control, OS kernel \\
        \hline
...
Docker volume misconfiguration pose a threat to a system two ways. This could lead to a denial-of-service or info leak
attack.
...
entire filesystem with data, preventing other users from writing data and potentially crippling the IO performance of the machine.
...
by a container. This could still result in a single container filling up the partition and denying service for the other
containers. Preventing this would require setting a fixed size for Docker volumes or some other unique solution for the
container.

\section{AppArmor profiles and seccomp configurations}

Containers are more susceptible to issues since the OS is not fully virtualized as it is within a hypervisor.
Using Linux's security namespaces is absolutely necessary to ensure that the containers are not able to access the full
resources of the host. Taking a look at how Docker handles this shows a good default setup.

\subsection {Docker and Linux Namespaces}

A container is not a jailed process, but has extra security to ensure that this process is separated from the rest of the
host's processes. Linux namespaces allow separation of resources for a well-defined set of processes, which is a
necessity for containers to run without full virtualization. Unless otherwise specified, each Docker container will be
created with its own namespace. We can test the namespace separation for a set of containers if we know their PID's.
If the namespaces are not set up correctly, we can alert the user of the issue. An analysis of container security issues
by Sultan, Ahmad, and Dimitriou found that most container security considerations are resolved by features in the Linux
kernel, chiefly namespaces and AppArmor settings~\cite{SultanSari2019CSIC}. They also found that protection of the
container from the host itself can be achieved through hardware means. This shows that as long as sane defaults are not
removed, security between containers or between the container and host are not an issue of concern for the user of the
container.
...
through which communication is necessary.

We can check this easily using the Docker client. If a container is using the default network, the user will be prompted
to create a network for that container.
