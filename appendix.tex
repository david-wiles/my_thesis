...
func fatal(err error) {
	// Set up terminal UI for errors
	errorStyle := lipgloss.NewStyle().
		Bold(true).
		MarginTop(2).
		Foreground(lipgloss.Color("1"))

	_, _ = fmt.Println(errorStyle.Render("Fatal: " + err.Error()))
}
...
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err := scanner.Fix(autoFix); err != nil {
			fatal(err)
			return
		}
...
func (c *CustomScanner) Fix(autoFix bool) error {
	if len(c.Issues) > 0 {
		c.Issues.PrintIssues("Issues Detected")
		return c.Issues.Fix(autoFix)
	}
...
func (list IssueList) PrintIssues(header string) {
	headerTemplate := lipgloss.NewStyle().
		Bold(true).
		MarginTop(2).
		MarginBottom(2)

	// Print header
	_, _ = fmt.Println(headerTemplate.Render(header))

	styleTemplate := lipgloss.NewStyle().
		MarginLeft(2).
		MarginRight(2)
	...
	for _, issue := range list {
		...
		renderString += issue.RenderTitle(styleTemplate) + "\n"

	}

	_, _ = fmt.Println(renderString)
}
...
func (list IssueList) Fix(autoFix bool) error {
	for _, issue := range list {
		if autoFix && issue.Solution != nil {
			if err := issue.Solution(); err != nil {
				return err
			}
		} else {
			if err := issue.PromptFix(); err != nil {
				return err
			}
		}
	}
...
// RenderInfo will return a string containing the issue's message
func (issue Issue) RenderInfo(template lipgloss.Style) string {
	return template.Inherit(issue.messageStyle).Render(issue.message)
}

// RenderTitle will generate a string using lipgloss based on the issue's Title
func (issue Issue) RenderTitle(template lipgloss.Style) string {
	return template.Inherit(issue.messageStyle).Render(issue.title)
}
...
func (issue Issue) PromptFix() error {
	...
	if issue.Solution != nil {
		...
		_, _ = fmt.Println(issue.promptStyle.Render(issue.prompt + "\n" + "Do you wish to fix these Issues? [Y/n]"))
...
func MinorIssue(title, message string) *Issue {
	return &Issue{
		...
		promptStyle:  lipgloss.NewStyle().PaddingTop(1).BorderTop(true).BorderStyle(lipgloss.NormalBorder()),
	}
}
...
func WarningIssue(title, message string) *Issue {
	return &Issue{
		...
		title:        title,
		messageStyle: lipgloss.NewStyle().Foreground(lipgloss.Color("1")),
		promptStyle:  lipgloss.NewStyle().Margin(1),
	}
}
...
func SevereIssue(title, message string) *Issue {
	return &Issue{
		...
		message:      message,
		title:        title,
		messageStyle: lipgloss.NewStyle().Foreground(lipgloss.Color("#ffcc2b")).Bold(true),
		promptStyle:  lipgloss.NewStyle().Margin(1).Background(lipgloss.Color("3")),
	}
}
